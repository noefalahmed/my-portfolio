...
/* ── Main dashboard V3 ── */
const CimplAccountsDashboardV3: React.FC = () => {
  ...
  const visibleRows = cardFilter
    ? ROWS_V3.filter(r => r.status === cardFilter)
    : ROWS_V3
    ...
    if (row.subRows.length > 0) {
      setSelectedSubs(prev => {
        ...
        if (isSelected) {
          ...
          next.set(i, new Set(row.subRows.map((_, j) => j)))
        }
        return next
      })
    }
              ...
              <table className={styles.table}>
                <thead>
                  <tr>
                    ...
                    {/* chevron col — spacer in main header */}
                    <th className={v3.chevronTh} />
                    {COLUMNS.map(col => (
                      <th key={col}>
                  ...
                  {visibleRows.map((row, i) => {
                    ...
                    return (
                      <React.Fragment key={i}>
                        ...
                        >
                          {/* checkbox col */}
                          <td className={v3.checkboxTd} onClick={e => e.stopPropagation()}>
                            <input
                              type="checkbox"
                              className={styles.checkbox}
                        ...
                        {/* Sub-section — nested table starting at chevron position */}
                        {isExpandable && isExpanded && (
                          <tr className={v3.subSectionRow}>
                            <td colSpan={14} className={v3.subSection}>
                              <table className={v3.subTable}>
                                <thead>
                                  <tr className={v3.subHeaderRow}>
                                    <td className={v3.subSpacer} />
                                    <td className={v3.subCheckCell} />
                                    {COLUMNS.map(col => (
                                      <td key={col} className={v3.subHeaderCell}>
                                        <span className={styles.thInner}>
                                          {col}
                                          <MI name="swap_vert" style={{ fontSize: 16, color: '#6B7786' }} />
                                        </span>
                                ...
                                <tbody>
                                  {row.subRows.map((subRow, j) => (
                                    ...
                                    >
                                      <td className={v3.subSpacer} />
                                      <td className={v3.subCheckCell} onClick={e => e.stopPropagation()}>
                                        <input
                                          type="checkbox"
                                          className={styles.checkbox}
...
export default CimplAccountsDashboardV3
